  ...
  <main class="container page">
    <div v-if="hasLoaded" class="d-flex flex-column flex-grow-1">
      <div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-start gap-3">
        ...
        <CreateSpell class="mb-3" @created="onCreate" @error="handleError" />
      </div>
...
import CreateSpell from "@/components/spells/CreateSpell.vue";
...
import type { SearchSpellsPayload, Spell, SpellSort } from "@/types/spells";
...
const title = computed<string>(() => t("spells.title"));

const hasFilters = computed<boolean>(() => typeof tier.value === "number" || Boolean(search.value));

const sortOptions = computed<SelectOption[]>(() =>
  orderBy(
    Object.entries(tm(rt("spells.sort.options"))).map(([value, text]) => ({ text, value }) as SelectOption),
    "text",
  ),
);

function onCreate(spell: Spell): void {
  events.push("created");
  router.push({ name: "Spell", params: { id: spell.id } });
}

function clearFilters(): void {
  const query = { ...route.query, search: "", tier: "", page: 1 };
...
async function refresh(): Promise<void> {
  const payload: SearchSpellsPayload = {
    ids: [],
    search: {
      terms: search.value
        .split(" ")
    ...
    sort: sort.value ? [{ field: sort.value as SpellSort, isDescending: isDescending.value }] : [],
  ...
  timestamp.value = now;
  try {
    const results: SearchResults<Spell> = await searchSpells(payload);
    if (now === timestamp.value) {
      spells.value = [...results.items];
