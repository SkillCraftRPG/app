<template>
  <div class="h-100">
    <TarCard class="clickable h-100" @click="open">
      ...
      <TarProgress :aria-label="label" class="progress-intoxication mt-1" :label="progress.card.label" :value="progress.card.value" />
    </TarCard>
...
import IntoxicationField from "./IntoxicationField.vue";
...
watch(
  () => props.character,
  (character) => {
    current.value = character.intoxication;
    reinitialize();
  },
