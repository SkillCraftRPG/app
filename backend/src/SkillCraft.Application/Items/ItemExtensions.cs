...
internal static class ItemExtensions
{
  ...
  {
    long size = item.Name.Size + (item.Description?.Size ?? 0) + 8 + 8 + 4; // TODO(fpion): Properties
  ...
  {
    long size = item.Name.Length + (item.Description?.Length ?? 0) + 8 + 8 + 4; // TODO(fpion): Properties
