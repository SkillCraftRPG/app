...
internal static class LineageExtensions
{
  ...
  {
    long size = 4 + lineage.Name.Size + (lineage.Description?.Size ?? 0) + 32; // TODO(fpion): complete, include WorldId?
  ...
  {
    long size = 4 + lineage.Name.Length + (lineage.Description?.Length ?? 0) + 32; // TODO(fpion): complete, include WorldId?
