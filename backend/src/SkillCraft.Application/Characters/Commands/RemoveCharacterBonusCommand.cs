...
public record RemoveCharacterBonusCommand(Guid CharacterId, Guid BonusId) : Activity, IRequest<CharacterModel?>;

internal class RemoveCharacterBonusCommandHandler : IRequestHandler<RemoveCharacterBonusCommand, CharacterModel?>
{
  ...
  public RemoveCharacterBonusCommandHandler(
  ...
  public async Task<CharacterModel?> Handle(RemoveCharacterBonusCommand command, CancellationToken cancellationToken)
  {
    ...
    character.RemoveBonus(command.BonusId, command.GetUserId());
