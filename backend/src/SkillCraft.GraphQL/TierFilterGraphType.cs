...
internal class TierFilterGraphType : InputObjectGraphType<TierFilter>
{
  public TierFilterGraphType()
  {
    ...
    Field(x => x.Values, type: typeof(NonNullGraphType<ListGraphType<NonNullGraphType<IntGraphType>>>))
      .Description("The values compared to the tier of the entities. If should only contain values ranging from 0 to 3, each value should only be included once, and only one value should be included when not using the IN or NOT IN operator.");
    Field(x => x.Operator)
