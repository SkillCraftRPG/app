...
public class SetGiftCommandHandlerTests
{
  ...
  public SetGiftCommandHandlerTests()
  {
    ...
    _nature = new(_world.Id, new Name("Mystérieux"), _world.OwnerId);
  }

  [Fact(DisplayName = "It should nullify the gift.")]
  public async Task It_should_nullify_the_gift()
  {
    _nature.SetGift(_gift);
    Assert.Equal(_gift.Id, _nature.GiftId);

    SetGiftCommand command = new(_nature, Id: null);
    await _handler.Handle(command, _cancellationToken);

    Assert.Null(_nature.GiftId);
  }
  ...
  public async Task It_should_set_the_gift()
  {
    ...
    SetGiftCommand command = new(_nature, _gift.EntityId);
    await _handler.Handle(command, _cancellationToken);
  ...
  public async Task It_should_throw_CustomizationIsNotGiftException_when_the_customization_is_not_a_gift()
  {
    ...
    SetGiftCommand command = new(_nature, customization.EntityId);
  ...
  {
    SetGiftCommand command = new(_nature, Guid.Empty);
