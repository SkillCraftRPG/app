...
public class PermissionServiceTests
{
  ...
  public async Task EnsureCanAsyncWorld_it_should_succeed_when_the_user_has_the_permission()
  {
    ...
    CreateOrReplaceWorldCommand command = new(_world.Id, new CreateOrReplaceWorldPayload(_world.Slug), Version: null);
  ...
  {
    CreateOrReplaceEducationCommand command = new(Id: null, new CreateOrReplaceEducationPayload("Classique"), Version: null);
    command.Contextualize(_otherUser, _world);
  ...
  {
    CreateOrReplaceWorldCommand command = new(Id: null, new CreateOrReplaceWorldPayload("ungar"), Version: null);
  ...
  [Fact(DisplayName = "EnsureCanPreviewAsync(EntityMetadata): it should succeed when the user has the permission.")]
  public async Task EnsureCanPreviewAsyncEntityMetadata_it_should_succeed_when_the_user_has_the_permission()
  {
    ...
    _permissionQuerier.Setup(x => x.HasAsync(_otherUser, _world, Action.Preview, entity.Type, null, _cancellationToken)).ReturnsAsync(true);

    await _service.EnsureCanPreviewAsync(query, entity, _cancellationToken);

    _permissionQuerier.Verify(x => x.HasAsync(_otherUser, _world, Action.Preview, entity.Type, null, _cancellationToken), Times.Once);
  }

  [Fact(DisplayName = "EnsureCanPreviewAsync(EntityMetadata): it should succeed when the user is the world owner.")]
  public async Task EnsureCanPreviewAsyncEntityMetadata_it_should_succeed_when_the_user_is_the_world_owner()
  {
    ReadLineageQuery query = new(Guid.NewGuid());
  ...
  [Fact(DisplayName = "EnsureCanPreviewAsync(EntityMetadata): it should throw ArgumentException when the type is world.")]
  public async Task EnsureCanPreviewAsyncEntityMetadata_it_should_throw_ArgumentException_when_the_type_is_world()
  {
    ReadWorldQuery query = new(_world.Id, _world.Slug);
    query.Contextualize(_user);

    EntityMetadata entity = new(WorldId, new EntityKey(EntityType.World, _world.Id), size: 5);
    var exception = await Assert.ThrowsAsync<ArgumentException>(async () => await _service.EnsureCanPreviewAsync(query, entity, _cancellationToken));
    Assert.StartsWith("The entity type must not be 'World'.", exception.Message);
    Assert.Equal("entity", exception.ParamName);
  ...
  public async Task EnsureCanPreviewAsyncEntityMetadata_it_should_throw_PermissionDeniedException_when_the_entity_does_not_reside_in_the_world()
  {
    ReadLineageQuery query = new(Guid.NewGuid());
    query.Contextualize(_otherUser, _world);

    EntityMetadata entity = new(WorldId.NewId(), new EntityKey(EntityType.Lineage, Guid.NewGuid()), size: 7);
    var exception = await Assert.ThrowsAsync<PermissionDeniedException>(async () => await _service.EnsureCanPreviewAsync(query, entity, _cancellationToken));
  ...
  [Fact(DisplayName = "EnsureCanPreviewAsync(EntityMetadata): it should throw PermissionDeniedException when the user does not have the permission.")]
  public async Task EnsureCanPreviewAsyncEntityMetadata_it_should_throw_PermissionDeniedException_when_the_user_does_not_have_the_permission()
  {
    ReadLineageQuery query = new(Guid.NewGuid());
    query.Contextualize(_otherUser, _world);
