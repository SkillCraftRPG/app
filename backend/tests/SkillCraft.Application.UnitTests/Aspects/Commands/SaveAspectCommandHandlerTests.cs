...
public class SaveAspectCommandHandlerTests
{
  ...
  {
    SaveAspectPayload payload = new(" Œil-de-lynx ")
    {
      Description = "    ",
    ...
    SaveAspectCommand command = new(parsed ? id : null, payload, Version: null);
    command.Contextualize(_world);

    SaveAspectResult result = await _handler.Handle(command, _cancellationToken);
    ...
    _aspectRepository.Verify(x => x.SaveAsync(
      It.Is<Aspect>(y => (!parsed || y.EntityId == id)
  ...
  public async Task It_should_replace_an_existing_aspect()
  {
    ...
    SaveAspectCommand command = new(_aspect.EntityId, payload, Version: null);
    command.Contextualize(_world);
    ...
    _aspectRepository.Verify(x => x.SaveAsync(
      It.Is<Aspect>(y => y.Equals(_aspect)
        && y.Name.Value == payload.Name.Trim() && y.Description == null
        && y.Attributes.Mandatory1 == payload.Attributes.Mandatory1 && y.Attributes.Mandatory2 == payload.Attributes.Mandatory2
        && y.Attributes.Optional1 == payload.Attributes.Optional1 && y.Attributes.Optional2 == payload.Attributes.Optional2
        && y.Skills.Discounted1 == payload.Skills.Discounted1 && y.Skills.Discounted2 == payload.Skills.Discounted2),
    ...
    VerifyStorage(_aspect.EntityId);
  }
  ...
  {
    SaveAspectCommand command = new(Guid.Empty, new SaveAspectPayload("Œil-de-lynx"), Version: 0);
    command.Contextualize(_world);
  ...
  {
    SaveAspectPayload payload = new();
    payload.Attributes.Mandatory1 = (Attribute)(-1);

    SaveAspectCommand command = new(Id: null, payload, Version: null);
  ...
  public async Task It_should_update_an_existing_aspect()
  {
    ...
    SaveAspectCommand command = new(_aspect.EntityId, payload, reference.Version);
    command.Contextualize(_world);
  ...
  private void VerifyStorage(Guid? id)
  {
    _storageService.Verify(x => x.EnsureAvailableAsync(
      It.Is<EntityMetadata>(y => y.WorldId == _world.Id && y.Type == EntityType.Aspect && (id == null || y.Id == id) && y.Size > 0),
      _cancellationToken), Times.Once);
