...
public class LineageTests
{
  ...
  [Fact(DisplayName = "It should throw InvalidParentLineageException when setting a parent with a parent.")]
  public void It_should_throw_InvalidParentLineageException_when_setting_a_parent_with_a_parent()
  {
    ...
    var exception = Assert.Throws<InvalidParentLineageException>(() => new Lineage(_lineage.WorldId, parent, _lineage.Name, new UserId(_lineage.CreatedBy)));
    Assert.Equal(_lineage.WorldId.ToGuid(), exception.WorldId);
    Assert.Equal(parent.EntityId, exception.LineageId);
